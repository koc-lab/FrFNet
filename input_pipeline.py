...
"""TFDS input pipelines for GLUE and C4 (Wikipedia dataset is used for FrFNet) datasets."""
...
def _c4_data_unbatched(tokenizer,
                       max_seq_length):
  """Yields examples from C4 (Wikipedia) corpus that have len(text) <= max_seq_length."""
  cls_id = tokenizer.PieceToId("[CLS]")
  sep_id = tokenizer.PieceToId("[SEP]")
  pad_id = tokenizer.pad_id()

  # ds = tfds.load(name="c4/realnewslike", split="train", shuffle_files=True)
...
def c4_masked_lm_inputs(
  ...
  while True:
    ...
    # To skip empty our erroneous instances.
    try:
        masked_lm_positions = np.random.choice(cand_indexes, num_to_predict, replace=False)
    except ValueError:
        print("Error Repeat: 'a' cannot be empty unless no samples are taken")
        print("cand_indexes:")
        print(cand_indexes)
        print("num_to_predict")
        print(num_to_predict)
        print("Passing to next example...")
        continue
